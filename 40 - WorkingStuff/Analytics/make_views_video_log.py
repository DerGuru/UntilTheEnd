"""Same as make_views_video.py but with a log-scale y-axis, to make relative
growth rates comparable across chapters with very different view magnitudes.
"""
import json
import re
...
OUT = Path(r"d:\UntilTheEnd\40 - WorkingStuff\Analytics\views_per_chapter_growth_log.mp4")
...
FLOOR = 1.0  # log scale can't show 0 -- clip empty/unpublished chapters to this
...
cumulative = np.cumsum(daily, axis=1).astype(float)
cumulative_display = np.maximum(cumulative, FLOOR)  # floor zeros for log display

y_max = cumulative[:, -1].max() * 1.3

fig, ax = plt.subplots(figsize=(16, 6), dpi=110)
x = np.arange(n)
bars = ax.bar(x, cumulative_display[:, 0], color=colors, width=1.0, edgecolor="none")
ax.set_yscale("log")
ax.set_ylim(FLOOR, y_max)
ax.set_xlim(-1, n)
ax.set_xlabel("Chapter (Embers 01 -> Clouds 68)")
ax.set_ylabel("Cumulative views (log scale)")
...
for day_i in range(0, n_days, STEP):
    for rect, h in zip(bars, cumulative_display[:, day_i]):
        rect.set_height(h)
    title_txt.set_text(f"Cumulative views per chapter (log scale) -- {all_days[day_i]}")
    fig.canvas.draw()
